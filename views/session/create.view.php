...
<div class="flex justify-center w-screen py-12 px-4 sm:px-6 lg:px-8">
  <div class="w-full max-w-xs space-y-8">
    ...
    <form class="mt-8 space-y-6" action="/session" method="POST">
      ...
      <div>
            ...
            <input id="username_email" name="username_email" type="text" autocomplete="username email" required
                class="relative block w-full appearance-none rounded-none rounded-t-md border border-gray-300 px-3 py-2 text-gray-900 placeholder-gray-500 focus:z-10 focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:text-sm"
                placeholder="Username or Email" value="<?= old('username_email') ?>">
        </div>
        ...
        <?php if (isset($errors['username_email'])) : ?>
            <p class="text-red-500 text-xs mt-2"> <?= $errors['username_email'] ?> </p>
        <?php endif; ?>
